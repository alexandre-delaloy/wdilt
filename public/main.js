...
| -- | ---------------- | ---------------- | ---------------- | ---------------- | ---------------- | ---------------- | ---------------- |\n`;
    ...
    fill() {
        ...
        for (let i = 0; i < this.weeks; i++)
            this.calendarTemplate.push([]);
        // for every empty days before first day, push an empty day space
        for (let i = 0; i < this.emptyDaysBeforeFirstDay; i++)
            this.calendarTemplate[0].push(this.space);
        ...
        for (let i = 0; i < this.calendarTemplate.length; i++) {
            ...
            for (let j = 0; j < 7; j++) {
                ...
                if (i <= this.weeks - 2 && this.day <= this.days)
                    this.addDay(i + 1);
                // and increment
                this.incDay();
            }
        ...
        for (let h = 0; h < this.emptyDaysAfterLastDay; h++)
            this.calendarTemplate[this.calendarTemplate.length - 1].push(this.space);
        // for every week, push all string in the calendarTemplate
        for (let i = 0; i < this.weeks; i++)
            this.finalTemplate += `|${this.calendarTemplate[i].join(`|`)}|\n`;
        // return the template
        return console.log(`\n${this.finalTemplate}`);
    }
};
